 ...
 * <li>{@link #setCacheFullRemoveType(CacheFullRemoveType)} set remove type when
 * cache is full</li>
 * <li>{@link #setValidTime(long)} set valid time of elements in cache, in mills
 * </li>
 * <li>{@link #put(Object, CacheObject)} or {@link #put(Object, Object)} put
 * element</li>
...
public class SimpleCache<K, V> implements Cache<K, V>, Serializable {
	...
	protected boolean isExpired(CacheObject<V> obj) {
		return validTime != -1 && (obj == null || (obj.isExpired() && !obj.isForever())
				|| (obj.getEnterTime() + validTime) < System.currentTimeMillis());
	}
