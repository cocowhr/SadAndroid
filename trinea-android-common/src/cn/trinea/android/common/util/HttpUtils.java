...
public class HttpUtils {
    ...
    public static HttpResponse httpGet(HttpRequest request) {
        ...
        try { 
        	...
        	int index = fUrl.indexOf('?');
        	
            URL url = new URL(fUrl.substring(0, index));

            try {
            	// 2015-02-21 
            	// there is no need to use Httpget
             ...
             //System.out.println(fUrl + url);
             
                con.getOutputStream().write(fUrl.substring(index + 1).getBytes());
                /////////
